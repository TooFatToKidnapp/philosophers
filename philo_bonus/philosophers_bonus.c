...
/*   Updated: 2022/04/13 13:13:28 by aabdou           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	all->philo = malloc(sizeof(t_philo) * all->data->nb_of_philo);
	if (all->philo == NULL)
		return (1);
...
int	init_sems(t_all *all)
{
	...
	if (all->lock == NULL)
		return (put_err("Memory's not been allocated", 0));
	sem_unlink("output");
	all->lock->output = sem_open("output", O_CREAT, 0644, 1);
	if (all->lock->output == SEM_FAILED)
		return (put_err("Semaphore failed to open", 0));
	sem_unlink("forks");
	all->lock->forks = sem_open("forks", O_CREAT, 0644, \
	all->data->nb_of_philo / 2);
